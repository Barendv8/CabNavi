    ...
    private:
        ...
        static const std::unordered_map<std::string, const char *> &Engels()
        {
            ...
            // --- Added after converting the code ----------------------
                ...
                // --- v1.1: eenheden, spelnaam, webhookteksten ---
                // --- v1.2: VTC-bron en dispatch ---
                { "Positie live delen met mijn VTC", "Share my live position with my VTC" },
                { "Elke 10 s je plek naar het adres hierboven", "Every 10 s your position to the address above" },
                { "VTLog", "VTLog" },
                { "Optioneel: VTC-sleutel voor leden en live", "Optional: VTC key for members and live" },
                // --- v1.2: kosten per liter, leeg rijden ---
                { "Formaat", "Format" },
                { "Reserve als tankbeurt en plek onbekend zijn", "Fallback when no refuel and no place is known" },
                { "CabNavi (open ritformaat)", "CabNavi (open trip format)" },
                { "Envelop met handtekening", "Envelope with signature" },
                { "Geheim van de hub, voor de handtekening", "The hub's secret, for the signature" },
                { "GETANKT", "FUEL BOUGHT" },
                { "LEEG GEREDEN", "EMPTY RUNNING" },
                { "Leeg gereden", "Empty run" },
                { "Netto na leeg rijden", "Net after empty running" },
                { "Prijs hier", "Price here" },
                { "Prijs ingesteld", "Price as set" },
                { "Prijs laatste tankbeurt", "Price at last refuel" },
                { "eigen garage", "own garage" },
                { "Waar leeft je bedrijf? Dat vult de VTC-tab.", "Where does your company live? It fills the VTC tab." },
                { "Bron", "Source" },
                { "Geen", "None" },
                { "CabNavi bot (dispatch)", "CabNavi bot (dispatch)" },
                { "Trucky", "Trucky" },
                { "Horizon Dispatch", "Horizon Dispatch" },
                { "TruckersHub", "TruckersHub" },
                { "Publieke pk_-sleutel (Dashboard > API)", "Public pk_ key (Dashboard > API)" },
                { "Bedrijfstoken: geheim, kan leden wissen", "Company token: secret, can remove members" },
                { "Optioneel: bedrijfstoken voor evenementen", "Optional: company token for events" },
                { "Vindt je account zelf via je Steam-ID", "Finds your account by itself via your Steam ID" },
                { "Gebruikt adres en sleutel van de VTC-webhook", "Uses the VTC webhook address and key" },
                { "BRON", "SOURCE" },
                { "IK", "ME" },
                { "Ophalen...", "Fetching..." },
                { "OPDRACHTEN", "JOBS" },
                ...
                { "Online: ", "Online: " },
                { "Elke afgeronde rit als JSON naar je VTC", "Every finished trip as JSON to your VTC" },
                { "Ritten doorsturen", "Forward trips" },
                { "Adres (https), van je VTC gekregen", "Address (https), provided by your VTC" },
                { "Sleutel (gaat mee als Authorization: Bearer)",
                  "Key (sent as Authorization: Bearer)" },
                { "Eenheden", "Units" },
                { "Automatisch (volgt het spel)", "Automatic (follows the game)" },
                { "Metrisch (km, liter)", "Metric (km, litres)" },
                { "Imperiaal (mi, gallon)", "Imperial (mi, gallons)" },
                { "mi te gaan", "mi to go" },
                { "mi totaal", "mi total" },
                { "gal/uur stat.", "gal/hour idle" },
                { "gal/uur - stationair", "gal/hour - idling" },
                { " per liter", " per litre" },
                { " per gallon", " per gallon" },
                { "%s %.2f per liter (zelf ingesteld)", "%s %.2f per litre (set by you)" },
                { "%s %.2f per gallon (zelf ingesteld)", "%s %.2f per gallon (set by you)" },
                { "Uit brandstofprijzen.json: %s %.2f", "From brandstofprijzen.json: %s %.2f" },
                { "Getankt: %dx, %.0f liter, %s %.0f", "Refuelled: %dx, %.0f litres, %s %.0f" },
                { "Getankt: %dx, %.0f gallon, %s %.0f", "Refuelled: %dx, %.0f gallons, %s %.0f" },
                { "%s -- %.0f mi -- rond %s", "%s -- %.0f mi -- around %s" },
                { "%s -- %.0f mi vanaf start", "%s -- %.0f mi from the start" },
                { "%s | %.0f mi gepland", "%s | %.0f mi planned" },

                { "Vrachtrit ", "Freight job " },
                { "Buslijn ", "Bus line " },
                { "afgerond", "completed" },
                { "geannuleerd", "cancelled" },
                { "Van", "From" },
                ...
                { "Afstand", "Distance" },
                { " (gepland ", " (planned " },
                { "Duur (speltijd)", "Duration (game time)" },
                { "Opbrengst", "Revenue" },
                { "Brandstofkosten", "Fuel cost" },
                { "Boetes (", "Fines (" },
                { "Netto", "Net" },
                ...
                { " meer)", " more)" },
                { " | lading ", " | cargo " },
                { "Reden annulering", "Cancellation reason" },
                { "CabNavi is verbonden -- als je dit ziet, werkt de webhook!",
                  "CabNavi is connected -- if you can see this, the webhook works!" },
